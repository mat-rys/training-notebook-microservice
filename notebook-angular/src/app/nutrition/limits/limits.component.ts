...
import { Chart, PieController, ArcElement, Tooltip, Legend, LineController, LineElement, PointElement, LinearScale, Title } from 'chart.js';
import { Subject } from 'rxjs';
import { debounceTime } from 'rxjs/operators';

Chart.register(PieController, ArcElement, Tooltip, Legend, LineController, LineElement, PointElement, LinearScale, Title);
...
export class LimitsComponent implements OnInit {
  ...
  loadLimitsProfile() {
    this.limitService.loadLimitsProfile().subscribe(
      data => {
        this.limits = data;
        this.createPieChart();  // Initial chart creation
      },
      error => {
        if (error.status === 404) {
        ...
        this.limitService.updateLimitsProfile('limitCarbs', updatedCarbs).subscribe(() => {
          this.limitService.updateLimitsProfile('limitFats', updatedFats).subscribe(() => {
            this.limitService.updateLimitsProfile('limitProteins', updatedProteins).subscribe(() => {
              this.loadLimitsProfile();  // Load updated limits
              this.createPieChart();     // Update chart
  ...
  createPieChart() {
    ...
    const percentageData = data.map(value => (total ? (value / total * 100).toFixed(2) : 0));

    const backgroundColors = labels.map(() => `rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 255)}, 0.9)`); 
